...
public class ServiceProduit implements IService<produit>{
 ...
 public List<produit> Search(String nom) {
        ...
        try {
           pst=conn.prepareStatement("SELECT * from produit WHERE nom like ?");
              nom+= "%";
                pst.setString(1,nom);
              rs=pst.executeQuery();
            while(rs.next())
            {
                list.add(new produit(rs.getInt("id"),rs.getString(2),rs.getString("image"),rs.getInt(4),rs.getString(5),new categorie(rs.getInt(6),nom),rs.getInt(7),rs.getInt(8)));
            }
        } catch (SQLException ex) {
    ...
    public produit getOne(int id) {
        String req="SELECT  p.id id_produit, p.nom nom_produit, p.image,p.prix ,"
                + "p.description, p.id_cat, c.nom nom_categorie, p.stock,p.code FROM produit p JOIN categorie c on p.id_cat=c.id where p.id=?";
        produit p=null;
        try {
